...
void Movie::startCapture(string name)
{
	if (!_running)
	{
		_running = true;
		_loop = new std::thread(Movie::capture, _streamcatcher, &_running, name);
	}
}
...
{	
	...
	VideoWriter video(name, CV_FOURCC('M','J','P','G'), fps, Size(width, height),true);
